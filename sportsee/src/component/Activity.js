...
function Activity(props) {
  ...
  useEffect(() => {
    FetchData("/user/" + props.id + "/activity").then(res => {
      setData(res.data);
    })
  },[])
  ...
  if(data){
    ...
    data.sessions.forEach((e, i) => {
      e.day = i;
      if(minKg === -1 || minKg > e.kilogram) minKg = e.kilogram;
      if(maxKg < e.kilogram) maxKg = e.kilogram;
    });
    ...
    return (
      ...
      <BarChart data={data.sessions} >
          ...
          <Legend layout="horizontal" verticalAlign="top" align="right" iconType="circle" iconSize="8"
            wrapperStyle={{paddingBottom: "30px", paddingRight: "15px"}}
            formatter={(value, entry, index) => <span className="legendText">{value}</span>} />
